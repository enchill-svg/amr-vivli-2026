...
**Status:** Implemented and passing all built-in checks. **Run the full platform with `python run_all.py`** from this folder — see [ONE_PIPELINE.md](ONE_PIPELINE.md). Parent repo map: [../REPO_STRUCTURE.md](../REPO_STRUCTURE.md).
...
Runs: `run_pipeline.py` (1–10) → integrity proof (19a, 19, 20, allocator, validator) → analytics (11–18) → **gated deliverables (18b)** → `verify_all_figures.py`.

### Integrity proof only

```bash
python run_evidence_gate.py
```

### Gated deliverables (after Step 18)

```bash
python src/step18b_gated_deliverables.py
```

| Path | Contents |
|------|----------|
| `deliverables/*_gated_v1.csv` | Public-facing rankings with `quality_gate` |
| `deliverables/gating_comparison_v1.csv` | Ungated vs gated summary |
| `deliverables/organism_drug_quality_gate_v1.csv` | Per organism–drug gate lookup |
| `bounds/identifiability_bounds_v1.csv` | Unified Manski bounds (ATLAS + SOAR) |
| `bounds/sampling_validation_summary_v1.csv` | PLEA/SOAR bias + coverage |
| `src/evidence_gate_core/gate_rules.py` | Gating rules for Step 18b |

### Success

```text
AMR Life Expectancy Platform: ALL STEPS PASSED
verify_all_figures: ALL CHECKS PASSED
```

**Planning docs:** [`docs/AMR_LIFE_EXPECTANCY_UNIFIED_PROJECT_BRIEF.md`](docs/AMR_LIFE_EXPECTANCY_UNIFIED_PROJECT_BRIEF.md), [`docs/LAYER_A_EVIDENCE_GATE_CORE_BUILD_PLAN.md`](docs/LAYER_A_EVIDENCE_GATE_CORE_BUILD_PLAN.md).

### Honest limitation (raw ATLAS)

The Vivli ATLAS export records carbapenemase **positives only** in gene columns (blank ≠ negative). Tier 1 lower bound P/N (~9.5%) recomputes from the repo; the concept-PDF Tier 2 upper **36.7%** requires a cleaned store with explicit negatives — documented in `docs/EVIDENCE_GATE_ESTIMANDS.md` §2.5.
