...
function UsersPage() {
  ...
  const exportCsv = () => {
    ...
    const rows = [
      cols.join(","),
      ...filtered.map((u) => cols.map((c) => JSON.stringify((u as never)[c] ?? "")).join(",")),
    ];
  ...
  return (
    <div className="space-y-5">
      <header className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-white/50 mt-1">
            Approve, reject, or suspend researcher access requests.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={exportCsv}
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-white/10 bg-white/[0.03] text-sm hover:bg-white/[0.06]"
          >
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-2 rounded-xl border border-cyan-400/20 bg-cyan-400/5 p-3 text-sm">
          ...
          <div className="ml-auto flex gap-2">
            <button
              onClick={() => setConfirm({ ids: [...selected], action: "approved" })}
              className="px-3 py-1.5 rounded-md bg-emerald-500/20 border border-emerald-400/30 text-emerald-200 text-xs inline-flex items-center gap-1"
            >
              <Check className="w-3 h-3" /> Approve
            </button>
            <button
              onClick={() => setConfirm({ ids: [...selected], action: "rejected" })}
              className="px-3 py-1.5 rounded-md bg-rose-500/20 border border-rose-400/30 text-rose-200 text-xs inline-flex items-center gap-1"
            >
              <X className="w-3 h-3" /> Reject
            </button>
            <button
              onClick={() => setConfirm({ ids: [...selected], action: "suspended" })}
              className="px-3 py-1.5 rounded-md bg-orange-500/20 border border-orange-400/30 text-orange-200 text-xs inline-flex items-center gap-1"
            >
          ...
          <tbody>
            ...
            {filtered.map((u) => (
              <tr key={u.id} className="border-t border-white/5 hover:bg-white/[0.02]">
                ...
                <td className="p-3 text-white/70 capitalize">
                  {(u.roles ?? []).join(", ").replace(/_/g, " ") || "—"}
                </td>
                <td className="p-3 text-white/60">{new Date(u.created_at).toLocaleDateString()}</td>
                <td className="p-3">
                  <StatusPill status={u.status as string} />
                </td>
                <td className="p-3 text-right">
                  <div className="inline-flex gap-1">
                    {u.status !== "approved" && (
                      <button
                        onClick={() => setConfirm({ ids: [u.id], action: "approved" })}
                        className="p-1.5 rounded-md hover:bg-emerald-500/20 text-emerald-300"
                        title="Approve"
                      >
                    ...
                    {u.status !== "rejected" && (
                      <button
                        onClick={() => setConfirm({ ids: [u.id], action: "rejected" })}
                    ...
                    {u.status !== "suspended" && (
                      <button
                        onClick={() => setConfirm({ ids: [u.id], action: "suspended" })}
                        className="p-1.5 rounded-md hover:bg-orange-500/20 text-orange-300"
                        title="Suspend"
                      >
      ...
      {confirm && (
        ...
        >
          <div
            className="max-w-md w-full rounded-2xl border border-white/10 bg-[#0A0D12] p-6"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-medium capitalize">
              {confirm.action} {confirm.ids.length} user{confirm.ids.length > 1 ? "s" : ""}?
            </h3>
            <p className="text-sm text-white/60 mt-2">
              {confirm.action === "approved" &&
                "Approved users get immediate access to protected surveillance data."}
              {confirm.action === "rejected" &&
                "Rejected users can no longer request access without administrator intervention."}
              {confirm.action === "suspended" &&
                "Suspended users lose platform access until they are reinstated."}
            </p>
            <div className="flex gap-2 mt-5 justify-end">
              ...
              <button
                onClick={() => setAction(confirm.ids, confirm.action)}
                className="px-4 py-2 rounded-lg bg-gradient-to-r from-cyan-400 to-emerald-400 text-black text-sm font-semibold"
              >
...
function StatusPill({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 rounded-full text-[10px] uppercase tracking-wider border ${map[status] ?? "bg-white/5 border-white/10"}`}
    >
      {status}
    </span>
  );
