...
export const Route = createFileRoute("/intel/$signal")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `Drill-down intelligence context for ${pretty(params.signal)} including outbreaks, variants and confidence.`,
      },
    ],
  }),
});
...
> = {
  "active-outbreaks": {
    ...
    eyebrow: "Epidemiological signal",
    ...
    narrative:
      "Outbreak count is climbing fastest in Central & Southern Africa, led by wastewater-confirmed Mpox clade Ib and post-cyclone cholera signals in DR Congo and Mozambique.",
  },
  "emerging-threats": {
    ...
    confidence: [
      { feature: "Anomaly z-score", weight: 0.4 },
      { feature: "Phylogenetic novelty", weight: 0.3 },
      { feature: "Mobility convergence", weight: 0.3 },
    ],
    narrative:
      "Two novel lineages crossed the emergence threshold this week; both show >0.85 phylogenetic novelty against the continental reference set.",
  },
  "variant-emergence-index": {
    title: "Variant Emergence Index",
    eyebrow: "Genomic signal",
    color: "oklch(0.78 0.19 60)",
    value: "3.7",
    trend: "+14.3% vs 14d",
    ...
    confidence: [
      { feature: "Sequence depth", weight: 0.35 },
      { feature: "Geographic diversity", weight: 0.25 },
      { feature: "S-gene mutation density", weight: 0.4 },
    ],
    narrative:
      "Emergence index trending up driven by S-gene mutation accumulation in JN.1 sublineages and growing Mpox Ib.3 dominance.",
  },
};

function spark(arr: number[]) {
  return arr.map((v, i) => ({ d: `t${i}`, v }));
}
...
function IntelDetail() {
  ...
  const sig = SIGNALS[signal] ?? {
    ...
    outbreaks: [],
    variants: [],
    confidence: [{ feature: "Composite model", weight: 1 }],
    narrative:
      "Detailed drill-down view. Underlying data sources are streaming and the AI engine is recalculating the confidence breakdown.",
  };
  ...
  return (
    <CommandPage
      ...
      kpis={[
        ...
        {
          label: "Linked outbreaks",
          value: String(sig.outbreaks.length),
          color: "var(--status-alert)",
        },
        {
          label: "Linked variants",
          value: String(sig.variants.length),
    ...
    >
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <GlassCard title="Linked outbreaks" subtitle="Underlying epidemiological events">
          {sig.outbreaks.length === 0 ? (
            <p className="text-xs text-muted-foreground">No active outbreaks linked.</p>
          ) : (
            <ul className="space-y-2">
              {sig.outbreaks.map((o) => (
                <li
                  key={o.country + o.pathogen}
                  className="rounded-lg border border-border/50 bg-background/30 p-3 text-sm"
                >
                  <div className="flex items-center gap-2">
                    <MapPin className="w-3.5 h-3.5 text-muted-foreground" />
                    <span className="flex-1">{o.country}</span>
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-[color:var(--status-alert)]/15 text-[color:var(--status-alert)]">
                      {o.score}
                    </span>
                  </div>
                  <div className="mt-1 text-[11px] text-muted-foreground flex items-center gap-2">
                    <FlaskConical className="w-3 h-3" /> {o.pathogen}
                    <span className="ml-auto inline-flex items-center gap-1 text-[color:var(--status-warn)]">
                      <ArrowUpRight className="w-3 h-3" />+{o.trend}%
                    </span>
                  </div>
                </li>
              ))}
        ...
        <GlassCard title="Linked variants" subtitle="Lineages amplifying this signal">
          {sig.variants.length === 0 ? (
            <p className="text-xs text-muted-foreground">No genomic linkage yet.</p>
          ) : (
            <ul className="space-y-2">
              {sig.variants.map((v) => (
                <li
                ...
                >
                  ...
                  <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
                    ...
                    <span className="ml-auto text-[color:var(--status-warn)]">
                      +{v.growth}% growth
                    </span>
                  </div>
                  <div className="mt-1 h-1.5 rounded-full bg-secondary/60 overflow-hidden">
                    <div
                      className="h-full"
                      style={{ width: `${v.freq * 100}%`, background: "oklch(0.7 0.2 300)" }}
                    />
                  </div>
                </li>
              ))}
            ...
            {sig.confidence.map((c) => (
              <li key={c.feature} className="text-xs">
                ...
                <div className="mt-1 h-1.5 rounded-full bg-secondary/60 overflow-hidden">
                  <div
                    className="h-full"
                    style={{ width: `${c.weight * 100}%`, background: sig.color }}
                  />
                </div>
              </li>
            ))}
      ...
      <GlassCard title="AI intelligence brief" subtitle="Narrative for response teams">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-[11px]">
          <Link
            to="/forecast-compare"
            className="px-3 py-1.5 rounded-md bg-[color:var(--accent)]/15 text-[color:var(--accent)] inline-flex items-center gap-1"
          >
            <Globe2 className="w-3 h-3" /> Compare countries
          </Link>
          <Link
            to="/alerts"
            className="px-3 py-1.5 rounded-md border border-border text-muted-foreground inline-flex items-center gap-1"
          >
            <Activity className="w-3 h-3" /> Open alert timeline
          </Link>
          <Link
            to="/forecasting"
            className="px-3 py-1.5 rounded-md border border-border text-muted-foreground inline-flex items-center gap-1"
          >
            <Brain className="w-3 h-3" /> Forecast engine
          </Link>
        </div>
      </GlassCard>
      ...
      <GlassCard title="Driver lattice" subtitle="What's pushing the signal right now">
        <div className="grid sm:grid-cols-2 gap-2 text-xs">
          {[
            ...
            <div
              key={d}
              className="rounded-lg border border-border/50 bg-background/30 p-2.5 flex items-center gap-2"
            >
              <Layers className="w-3 h-3 text-muted-foreground" />
              <span className="flex-1">{d}</span>
              <Zap className="w-3 h-3 text-[color:var(--accent)]" />
              <span className="font-mono text-muted-foreground">
                {Math.round(20 + ((i * 11) % 60))}%
              </span>
