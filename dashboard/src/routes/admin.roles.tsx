...
const ROLES = ["super_admin", "admin", "analyst", "public_health_officer", "researcher", "viewer"] as const;
...
const PERMISSIONS: { key: string; label: string; roles: readonly Role[] }[] = [
  ...
  { key: "alerts.manage", label: "Create & resolve alerts", roles: ["super_admin", "admin", "analyst"] },
  { key: "forecasts.run", label: "Run AI forecasting jobs", roles: ["super_admin", "admin", "analyst"] },
  { key: "data.upload", label: "Upload sequences / metadata", roles: ["super_admin", "admin", "analyst", "researcher"] },
  { key: "reports.publish", label: "Publish reports", roles: ["super_admin", "admin", "public_health_officer"] },
...
function RolesPage() {
  ...
  const { data: users = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      (r.data ?? []).forEach((row: { user_id: string; role: string }) => roleMap.set(row.user_id, [...(roleMap.get(row.user_id) ?? []), row.role]));
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="text-3xl font-light tracking-tight flex items-center gap-3"><KeyRound className="w-6 h-6 text-cyan-300" /> Roles & Permissions</h1>
        <p className="text-sm text-white/50 mt-1">Granular access control across the ViralTrack platform.</p>
      </header>
          ...
          <table className="w-full text-sm">
            <thead className="bg-white/[0.03] text-[10px] uppercase tracking-widest text-white/40">
              <tr>
                ...
                {ROLES.map((r) => (<th key={r} className="p-3 text-center capitalize">{r.replace(/_/g, " ")}</th>))}
              </tr>
            </thead>
            <tbody>
              {PERMISSIONS.map((p) => (
                <tr key={p.key} className="border-t border-white/5">
                  <td className="p-3 text-white/80">{p.label} <span className="text-white/30 text-[11px] ml-1">{p.key}</span></td>
                  {ROLES.map((r) => (
                    <td key={r} className="p-3 text-center">
                      {p.roles.includes(r) ? <span className="inline-block w-4 h-4 rounded-full bg-emerald-400/20 border border-emerald-400/40" /> : <span className="text-white/20">·</span>}
                    </td>
                  ))}
                </tr>
              ))}
