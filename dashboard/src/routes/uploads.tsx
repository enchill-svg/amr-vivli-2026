...
export const Route = createFileRoute("/uploads")({
  ...
  head: () => ({ meta: [
    ...
    { name: "description", content: "Securely upload and manage your private genomic datasets, trees, and metadata." },
  ] }),
});
      ...
      <div className="max-w-4xl mx-auto space-y-5">
        <header className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Private workspace — only you can see, download or delete these files.</p>
          </div>
        </header>
        <AuthGate><UploadsBody /></AuthGate>
      </div>
    </PageShell>
  );
}
...
function UploadsBody() {
  ...
  async function onDownload(name: string) {
    const { data, error } = await supabase.storage.from("user-uploads").createSignedUrl(`${prefix}/${name}`, 60);
    if (error) { setErr(error.message); return; }
  ...
  return (
    ...
    >
      ...
      {isLoading ? <p className="text-xs text-muted-foreground">Loading…</p> : files.length === 0 ? (
        <p className="text-xs text-muted-foreground">No files yet. Upload your first dataset to keep it private and tied to your account.</p>
      ) : (
            ...
            <li key={f.id ?? f.name} className="flex items-center gap-3 py-2.5 text-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {f.metadata?.size ? `${(f.metadata.size / 1024).toFixed(1)} KB · ` : ""}{f.created_at && new Date(f.created_at).toLocaleString()}
                </div>
              </div>
              <Button variant="ghost" size="sm" onClick={() => onDownload(f.name)}><Download className="w-4 h-4" /></Button>
              <Button variant="ghost" size="sm" onClick={() => onDelete(f.name)}><Trash2 className="w-4 h-4 text-[color:var(--status-alert)]" /></Button>
            </li>
          ))}
        </ul>
      )}
