import { Activity, Dna, Droplet, Globe2, ShieldAlert, Sparkles, FlaskConical, Brain } from "lucide-react";
import { useEffect, useState } from "react";

type Kpi = { icon: React.ElementType; label: string; value: number; suffix?: string; color: string; trend: "up" | "down" | "flat" };
...
function KpiCell({ k }: { k: Kpi }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 min-w-[170px]">
      <div className="w-9 h-9 rounded-lg grid place-items-center" style={{ background: `${k.color}1f`, color: k.color }}>
        <k.icon className="w-4 h-4" strokeWidth={1.75} />
      </div>
      <div className="min-w-0">
        <div className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground/80">{k.label}</div>
...
export function KpiStrip() {
  const kpis: Kpi[] = [
    ...
    { icon: Brain, label: "AI risk score", value: 74, suffix: "/100", color: "#facc15", trend: "up" },
  ];
  return (
...
export function InsightPanel({ items }: { items: { title: string; body: string; tag?: string }[] }) {
  return (
        ...
        {items.map((i) => (
          <li key={i.title} className="border-l-2 border-[color:var(--accent)]/60 pl-3">
            <div className="flex items-center gap-2">
              ...
              {i.tag && <span className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-secondary/60">{i.tag}</span>}
            </div>
