...
export function PageShell({ children, showTabs = true }: { children: React.ReactNode; showTabs?: boolean }) {
  return (
    <div className="min-h-screen overflow-hidden bg-background text-foreground">
      <div className="pointer-events-none fixed inset-0 opacity-60" style={{ background: "radial-gradient(800px 500px at 20% 10%, oklch(0.3 0.1 200 / 0.25), transparent 60%), radial-gradient(700px 400px at 90% 90%, oklch(0.3 0.12 25 / 0.16), transparent 60%)" }} />
      <div className="relative">
        ...
        <footer className="flex items-center justify-between border-t border-border/60 px-6 py-3 text-xs text-muted-foreground">
          <div className="flex items-center gap-1.5">AMR Life Expectancy Intelligence <Info className="h-3 w-3" /></div>
          <div>Publication-grade decision support · transparent assumptions</div>
          <div className="flex items-center gap-1.5">Data updated: live analytical views <span className="h-2 w-2 rounded-full bg-[color:var(--status-ok)]" /></div>
        </footer>
      </div>
    </div>
  );
