      ...
      <div className="space-y-5">
        <header className="relative overflow-hidden rounded-2xl border border-border bg-card/40 backdrop-blur-sm p-6">
          <div className="absolute inset-0 pointer-events-none opacity-50"
            style={{ background: "radial-gradient(500px 240px at 10% 20%, oklch(0.55 0.2 200 / 0.2), transparent 60%)" }}
          />
          <div className="relative flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl border border-[color:var(--accent)]/40 flex items-center justify-center bg-background/40"
              style={{ boxShadow: "0 0 18px oklch(0.78 0.18 200 / 0.25), inset 0 0 10px oklch(0.78 0.18 200 / 0.15)" }}>
              <Icon className="w-6 h-6 text-[color:var(--accent)]" />
            </div>
            <div className="flex-1">
              <div className="text-[10px] tracking-[0.2em] uppercase text-[color:var(--accent)]">{eyebrow}</div>
            ...
            <div className="relative mt-5 grid grid-cols-2 md:grid-cols-4 gap-3">
              {kpis.map((k) => (
                <div key={k.label} className="rounded-lg border border-border/60 bg-background/30 p-3">
                  <div className="text-2xl font-light" style={{ color: k.color ?? "var(--accent)" }}>{k.value}</div>
