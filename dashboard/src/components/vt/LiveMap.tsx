      ...
      {selected.length > 0 && (
        <div className="absolute top-3 left-3 z-[600] flex items-center gap-2 rounded-lg bg-card/90 border border-[color:var(--accent)]/50 px-3 py-1.5 backdrop-blur-sm text-[11px]">
          ...
          <span className="text-muted-foreground">
            marker{selected.length === 1 ? "" : "s"} selected for report
          </span>
        ...
        <HeatLayer
          points={filteredAlerts
            .map((a) => {
              ...
              if (!c) return null;
              return [c[0], c[1], SEVERITY_INTENSITY[a.severity] ?? 0.5] as [
                number,
                number,
                number,
              ];
              ...
              >
                <Tooltip direction="top">
                  <div className="text-xs">
                    ...
                    {s.population_served && (
                      <div className="opacity-70">
                        Pop. served: {s.population_served.toLocaleString()}
                      </div>
                    )}
                  </div>
                </Tooltip>
            ...
            >
              <Popup>
                <div className="text-xs space-y-1 min-w-[180px]">
                  ...
                  <div>
                    <span className="opacity-60">Pathogen:</span> {a.pathogen}
                  </div>
                  <div>
                    <span className="opacity-60">Country:</span> {a.country}
                  </div>
                  ...
                  {a.description && (
                    <div className="opacity-80 pt-1 border-t border-border/40">{a.description}</div>
                  )}
                  <button
                    onClick={() =>
                      toggle({
              ...
              <Tooltip direction="top">
                <div className="text-xs">
                  ...
                  <div className="opacity-70">
                    {a.pathogen} · {a.country}
                  </div>
                  <div className="opacity-60 capitalize">Severity: {a.severity}</div>
                  <div className="opacity-60">
                    {new Date(a.detected_at).toLocaleDateString()} · Click for details
                  </div>
                </div>
              </Tooltip>
            </CircleMarker>
