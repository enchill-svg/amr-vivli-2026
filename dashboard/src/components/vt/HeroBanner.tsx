...
export function HeroBanner() {
  const { data: stats } = useQuery({
    queryKey: ["hero-stats"],
    queryFn: async () => {
      const [{ count: alerts }, { count: sites }, { count: seq }, { data: highSev }] =
        await Promise.all([
          supabase
            .from("alerts")
  ...
  const riskColor =
    risk > 75 ? "var(--status-alert)" : risk > 50 ? "var(--status-warn)" : "var(--status-ok)";
      ...
      <div className="relative grid md:grid-cols-[1fr_280px] gap-6 p-6">
        <div className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground max-w-xl">
            Wastewater signals, viral sequencing, lineage tracking, and AI outbreak forecasting —
            fused into a single command center for public health response.
          </p>

          <div className="flex flex-wrap gap-2 pt-2">
            <Link
              to="/"
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-[color:var(--accent)] text-[color:var(--accent-foreground)] text-xs font-medium hover:opacity-90"
            >
              <Eye className="w-3.5 h-3.5" /> Explore Surveillance
            </Link>
            <Link
              to="/uploads"
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full border border-border bg-card/60 text-xs font-medium hover:border-[color:var(--accent)]/60"
            >
        ...
        <div className="relative rounded-xl border border-border bg-background/40 p-4 flex flex-col items-center justify-center">
          ...
          <div className="mt-2 text-[11px] tracking-wider font-medium" style={{ color: riskColor }}>
            {riskLabel}
          </div>
          ...
          <Link
            to="/forecasting"
            className="mt-3 text-[10px] tracking-wider text-[color:var(--accent)] inline-flex items-center gap-1"
          >
...
function RiskRing({ value, color }: { value: number; color: string }) {
  ...
  return (
    <svg width="150" height="150" viewBox="0 0 150 150">
      ...
      />
      <text x="75" y="78" textAnchor="middle" fontSize="32" fill={color} fontWeight="300">
        {value}
      </text>
      <text
        x="75"
