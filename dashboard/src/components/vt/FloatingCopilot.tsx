    ...
    <>
      {!open && (
        <button
          ...
          aria-label="Open AMR AI assistant"
          className="fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-full bg-[color:var(--accent)] px-4 py-3 text-[color:var(--accent-foreground)] shadow-[0_0_24px_-4px_var(--accent)] transition-transform hover:scale-105"
        >
        ...
        <div className="fixed bottom-5 right-5 z-50 flex max-h-[70vh] w-[92vw] flex-col rounded-xl border border-[color:var(--accent)]/40 bg-[color:var(--popover)]/95 shadow-[0_0_40px_-8px_var(--accent)] backdrop-blur-xl sm:w-[420px]">
          <header className="flex items-center justify-between border-b border-border/50 px-4 py-3">
            <div className="flex items-center gap-2">
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[color:var(--accent)]/20">
              ...
              <div>
                ...
                <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
                  Evidence · policy · explainability
                </div>
              </div>
            </div>
            <button
            ...
            {messages.length === 0 && (
              <div className="space-y-2 text-xs text-muted-foreground">
                <p className="text-foreground">
                  Ask questions about AMR burden, country risk, MIC trajectory, funding gaps, or
                  interventions.
                </p>
                  ...
                  {examples.map((q) => (
                    <li key={q}>
                      <button
                        ...
                        className="w-full rounded-md border border-border/60 px-2 py-1.5 text-left transition-colors hover:border-[color:var(--accent)]/60 hover:bg-secondary/40"
                      >
                        {q}
                      </button>
                    </li>
                ...
                <div key={m.id} className="flex gap-2">
                  <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-secondary">
                    {m.role === "user" ? (
                      ...
                      <Bot className="h-3 w-3 text-[color:var(--accent)]" />
                    )}
                  </div>
          ...
          >
            <input
              ...
              placeholder="Ask about countries, resistance, funding, interventions…"
              disabled={busy}
            ...
            <button
              ...
              className="rounded-md bg-[color:var(--accent)] px-3 text-[color:var(--accent-foreground)] disabled:opacity-50"
            >
