...
RISK_RANKING_METHODOLOGY = (
  "Equal-weight mean of within-pathogen-type percentile ranks (OECD/JRC composite "
  "indicator precedent — rank-based normalization, equal weights among available "
  "components). Components per Justice Section 7 Output 4: burden "
  "(higher= worse), evolutionary trajectory (more negative fitness slope = worse), "
  "health-system capacity (lower health expenditure % GDP = worse). Consumption "
  "excluded — no numeric ESAC-Net series locally. Vaccination excluded — not named "
  "in Justice Output 4 (line 109), unlike Stage 5 regression."
)
...
def build_identifiability_ledger() -> pd.DataFrame:
  ...
  rows.append(
    {
      "ledger_id": "consumption_numeric_series_absent",
      "pathogen_type": "both",
      "gap_category": "external_data_gap",
      ...
      "source_artifact": "bounds/external_join_bacterial_country_year_v1.csv",
      "n_strata_or_rows": 0,
      "metric_summary": "consumption_data_gap=no_numeric_esac_net_series_locally",
      "bound_lower_example": np.nan,
      "bound_upper_example": np.nan,
    ...
    return pd.Series(
      {
        ...
        if "consumption_data_gap" in g.columns
...
def build_country_risk_ranking(pathogen_type: str) -> pd.DataFrame:
  ...
  cy["health_capacity_risk_percentile"] = percentile_rank(-cy["health_expenditure_pct_gdp"])
  cy["consumption_risk_percentile"] = np.nan
  cy["n_risk_components_available"] = (
    cy["burden_risk_percentile"].notna().astype(int)
    + cy["trajectory_risk_percentile"].notna().astype(int)
    + cy["health_capacity_risk_percentile"].notna().astype(int)
  )
  cy["composite_risk_score"] = cy[
    ["burden_risk_percentile", "trajectory_risk_percentile", "health_capacity_risk_percentile"]
  ].mean(axis=1, skipna=True)
  cy = cy.sort_values("composite_risk_score", ascending=False)
  cy["risk_rank"] = range(1, len(cy) + 1)
...
def main() -> None:
  ...
  if "bacterial_beta_lactamase_detection_only" not in set(ledger["ledger_id"]):
    ...
    failed = True
  elif "consumption_numeric_series_absent" not in set(ledger["ledger_id"]):
    print("FAIL: identifiability ledger missing consumption gap entry.")
    failed = True
  else:
    print("PASS: identifiability ledger includes beta-lactamase and consumption gap entries.")
